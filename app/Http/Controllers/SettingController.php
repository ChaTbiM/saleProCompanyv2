...
class SettingController extends Controller
{
    ...
    public function generalSettingStore(Request $request)
    {
        ...
        $searchArray = array('APP_TIMEZONE='.config('app.timezone'));
    ...
    public function mailSettingStore(Request $request)
    {
        ...
        $path = '.env';
        $searchArray = array('MAIL_HOST="'.config('mail.host').'"', 'MAIL_PORT='.config('mail.port'), 'MAIL_FROM_ADDRESS="'.config('mail.from.address').'"', 'MAIL_FROM_NAME="'.config('mail.from.name').'"', 'MAIL_USERNAME="'.config('mail.username').'"', 'MAIL_PASSWORD="'.config('mail.password').'"', 'MAIL_ENCRYPTION="'.config('mail.encryption').'"');
        //return $searchArray;
